...
Neighbor::~Neighbor()
{
...
#ifdef N_HBW
  if(neighbors) memkind_free(MEMKIND_HBW, neighbors);
#else
  ...
  if(bins) memkind_free(MEMKIND_HBW, bins);
#else
  if(bins) free(bins);
#endif
}
...
void Neighbor::build(Atom &atom)
{
  ...
  if(nall > nmax) {
    ...
    neighbors = (int*)memkind_malloc(MEMKIND_HBW, nmax * maxneighs * sizeof(int*));
#else
    if(neighbors) free(neighbors);
...
void Neighbor::binatoms(Atom &atom)
{
  ...
  while(resize > 0) {
    ...
    for(int i = 0; i < nall; i++) {
      ...
      if(bins && bincount[ibin] < atoms_per_bin) {
        int ac;
#ifdef OpenMP31
  ...
  //  #pragma omp barrier
    ...
    if(resize) {
#ifdef BIN_HBW
      memkind_free(MEMKIND_HBW, bins);
      atoms_per_bin *= 2;
      bins = (int*) memkind_malloc(MEMKIND_HBW, mbins * atoms_per_bin * sizeof(int));
