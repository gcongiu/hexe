...
#ifdef X_HBW
    destroy_2d_MMD_float_array_fast(x);
#else
    destroy_2d_MMD_float_array(x);
#endif
#ifdef V_HBW
    destroy_2d_MMD_float_array_fast(v);
#else
    destroy_2d_MMD_float_array(v);
#endif
#ifdef F_HBW
    destroy_2d_MMD_float_array_fast(f);
#else
...
void Atom::growarray()
{
  ...
  nmax += DELTA;
#ifdef X_HBW
  x = (MMD_float**) realloc_2d_MMD_float_array_fast(x, nmax, 3, 3 * nold);
#else
  x = (MMD_float**) realloc_2d_MMD_float_array(x, nmax, 3, 3 * nold);
#endif
#ifdef V_HBW
  v = (MMD_float**) realloc_2d_MMD_float_array_fast(v, nmax, 3, 3 * nold);
#else
  v = (MMD_float**) realloc_2d_MMD_float_array(v, nmax, 3, 3 * nold);
#endif
#ifdef F_HBW
  f = (MMD_float**) realloc_2d_MMD_float_array_fast(f, nmax, 3, 3 * nold);
#else 
...
MMD_float** Atom::realloc_2d_MMD_float_array_fast(MMD_float** array,
    int n1, int n2, int nold)

{
  MMD_float** newarray;

  newarray = create_2d_MMD_float_array_fast(n1, n2);
...
MMD_float** Atom::create_2d_MMD_float_array_fast(int n1, int n2)
{
  int ALIGN = 16;
  MMD_float** array;
  MMD_float* data;
  ...
  array = (MMD_float**) malloc(n1 * sizeof(MMD_float*));
  data = (MMD_float*) memkind_malloc(MEMKIND_HBW, (n1 * n2 + 1024 + 1) * sizeof(MMD_float));
  long mask64 = 0;
...
void Atom::destroy_2d_MMD_float_array_fast(MMD_float** array)
{

  if(array != NULL) {
    //free(array[0]);
//    memkind_free(MEMKIND_HBW, array);
  }
}
