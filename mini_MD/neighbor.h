...
class Neighbor
{
  ...
  private:
    ...
    size_t nmax;                        // max size of atom arrays in neighbor
    int* bincount;                    // ptr to 1st atom in each bin
    int* bins;                       // ptr to next atom in each bin
    size_t atoms_per_bin;

    int nstencil;                    // # of bins in stencil
    int* stencil;                    // stencil list of bin offsets

    size_t mbins;                       // binning parameters
    size_t mbinx, mbiny, mbinz;
    size_t mbinxlo, mbinylo, mbinzlo;
