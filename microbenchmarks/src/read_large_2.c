...
#pragma omp parallel
...
#else
    fieldB = memkind_malloc(MEMKIND_DEFAULT, 2*sizeof(double)*(SIZE+2*1024*1024));
    fieldA = memkind_malloc(MEMKIND_HBW, 2*sizeof(double)*(SIZE+2*1024*1024));
#endif
    ...
    size_t prefetch_size = sizeof(double)*(POOL_SIZE + 2* 1024*1024);

    printf("block size %u\n, prefetch size %u\n", block, prefetch_size/(1024));
    int q = 0;
...
#pragma omp parallel for reduction(+ :goal) firstprivate(k)
            for(i = 0; i< POOL_SIZE; i++) {
                fieldB[i+POOL_SIZE*k]=(fieldA[k*POOL_SIZE+i]+fieldA[k*POOL_SIZE+i+1024*1024]
+fieldA[k*POOL_SIZE+i+2*1024*1024])*02.43;
            }
        }
    }
