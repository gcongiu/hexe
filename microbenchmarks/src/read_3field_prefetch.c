...
hexe_alloc_pool((POOL_SIZE+2*1024*1024)*sizeof(double), 2);
hexe_start();




fieldA = memkind_malloc(MEMKIND_DEFAULT, 2*sizeof(double)*(SIZE+2*1024*1024));
...
double *cache; // = memkind_malloc(MEMKIND_DEFAULT, prefetch_size);
...
#pragma omp parallel for firstprivate(k)  
        for(i = 0; i< POOL_SIZE; i++) {
            fieldB[i+POOL_SIZE*k]=(cache[i]+cache[i+1024*1024]+cache[i+2*1024*1024])*02.43;
        }
    }
}
