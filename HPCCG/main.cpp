...
/*before we start computing, distribute the threads */
#ifdef USING_OMP
...
#pragma omp parallel
   ...
   cpusets = (hwloc_cpuset_t *) malloc((threads+16)*sizeof(hwloc_bitmap_t));
    hwloc_distrib(topology, roots, nroots,cpusets, threads+16, INT_MAX, 0); 
   prefetcher=new Prefetch(threads, 16, cpusets, topology);
    prefetcher->alloc_pool(1024*1024*1024, 2);
    printf("Here \n");
#else
...
#endif
#ifdef PREFETCH
  ierr = HPCCG( A, b, x, max_iter, tolerance, niters, normr, times, prefetcher);
#else
