...
int HPCCG(HPC_Sparse_Matrix * A,
	  ...
	  double * times, Prefetch* prefetcher)
#else
int HPCCG(HPC_Sparse_Matrix * A,
	  const double * const b, double * const x,
	  const int max_iter, const double tolerance, int &niters, double & normr,
...
#ifdef MEMKIND
double * p =(double*) memkind_malloc(MEMKIND_P, ncol*sizeof(double));; // In parallel case, A is rectangular
  double * Ap =(double*) memkind_malloc(MEMKIND_AP, nrow*sizeof(double));
  ...
  int print_freq = 10000; //max_iter/10; 
...
#endif
#ifdef PREFETCH
  TICK(); HPC_sparsemv(A, p, Ap, prefetcher); TOCK(t3);
#else
...
#endif
#ifdef PREFETCH
      TICK(); HPC_sparsemv(A, p, Ap, prefetcher); TOCK(t3); // 2*nnz ops
#else
...
#endif
#ifdef MEMKIND
	memkind_free(MEMKIND_HBW, p);
	memkind_free(MEMKIND_HBW, Ap);
#else
  delete [] p;
  delete [] Ap;
#endif
